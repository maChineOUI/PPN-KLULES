   ...
   m_refdens(Real_t(1.0))
{
   ...
   m_conn.m_regNumList = Kokkos::View<Index_t*, Kokkos::HostSpace>("regNumList", numElem()) ;  // host-only
   ...
   // Use Kokkos::deep_copy(view, scalar) — direct host operator[]  on a
   // Kokkos::View is illegal when the default memory space is CudaSpace.
   Kokkos::deep_copy(m_elems.m_p,   Real_t(0.0)) ;
   Kokkos::deep_copy(m_elems.m_q,   Real_t(0.0)) ;
   Kokkos::deep_copy(m_elems.m_ss,  Real_t(0.0)) ;
   Kokkos::deep_copy(m_elems.m_v,   Real_t(1.0)) ; // v initializes to 1.0, not 0.0!
   ...
   // e and nodalMass are fully initialised in the field-data block below.
   ...
   // All Views are in device memory (CudaSpace on GPU), so use host mirrors
   // for the computation, then deep_copy the results to the device.
   {
      ...
      // volo_h(0) is already filled above — no device read needed
...
Domain::SetupThreadSupportStructures()
{
  ...
  if (numthreads > 1) {
    // m_nodelist lives in device memory — bring it to a host mirror first.
    auto nodelist_h = Kokkos::create_mirror_view(m_conn.m_nodelist) ;
    Kokkos::deep_copy(nodelist_h, m_conn.m_nodelist) ;
    ...
    // Allocate device Views; build CSR row-pointer on a host mirror.
    m_conn.m_nodeElemStart = Kokkos::View<Index_t*>("nodeElemStart", numNode()+1) ;
    auto nes_h = Kokkos::create_mirror_view(m_conn.m_nodeElemStart) ;

    nes_h(0) = 0 ;
    for (Index_t i=1; i <= numNode(); ++i) {
      nes_h(i) = nes_h(i-1) + nodeElemCount[i-1] ;
    }

    // Column-index array — size determined from host mirror.
    m_conn.m_nodeElemCornerList =
        Kokkos::View<Index_t*>("nodeElemCornerList", nes_h(numNode())) ;
    auto necl_h = Kokkos::create_mirror_view(m_conn.m_nodeElemCornerList) ;
	...
	Index_t m      = nl[j] ;
	Index_t k      = i*8 + j ;
	Index_t offset = nes_h(m) + nodeElemCount[m] ;
	necl_h(offset) = k ;
	++(nodeElemCount[m]) ;
      }
    }

    Index_t clSize = nes_h(numNode()) ;
    for (Index_t i=0; i < clSize; ++i) {
      Index_t clv = necl_h(i) ;
    ...
    Kokkos::deep_copy(m_conn.m_nodeElemStart,      nes_h) ;
    Kokkos::deep_copy(m_conn.m_nodeElemCornerList, necl_h) ;
  }
}
...
{
  // symmX/Y/Z are device Views — fill via host mirrors.
...
{
   // Connectivity Views live in device memory — fill via host mirrors.
...
{
  // Connectivity Views already contain interior values written by
  // SetupElementConnectivities (on device).  Pull them to host mirrors,
  // apply boundary patches, then push back.
  auto lxim_h   = Kokkos::create_mirror_view(m_conn.m_lxim) ;
  auto lxip_h   = Kokkos::create_mirror_view(m_conn.m_lxip) ;
  auto letam_h  = Kokkos::create_mirror_view(m_conn.m_letam) ;
  auto letap_h  = Kokkos::create_mirror_view(m_conn.m_letap) ;
  ...
  Kokkos::deep_copy(elemBC_h, Int_t(0)) ;
  ...
  for (Index_t i=0; i<edgeElems; ++i) {
	...
	elemBC_h(rowInc+j+numElem()-edgeElems*edgeElems) |=
	  ZETA_P_FREE;
      }
      else {
	elemBC_h(rowInc+j+numElem()-edgeElems*edgeElems) |=
