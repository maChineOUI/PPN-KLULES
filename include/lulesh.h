...
class Domain {
   ...
   // by value before launching KOKKOS_LAMBDA (GPU requires [=] capture).
   //
   ...
   struct ElemState {
      ...
      struct EosTemps {
         ...
         void reserve(Index_t n) {
            ...
            delvc        = Kokkos::View<Real_t*>("eos_delvc",        n);
            p_old        = Kokkos::View<Real_t*>("eos_p_old",        n);
            q_old        = Kokkos::View<Real_t*>("eos_q_old",        n);
            compression  = Kokkos::View<Real_t*>("eos_compression",  n);
            compHalfStep = Kokkos::View<Real_t*>("eos_compHalfStep", n);
            qq_old       = Kokkos::View<Real_t*>("eos_qq_old",       n);
            ql_old       = Kokkos::View<Real_t*>("eos_ql_old",       n);
            work         = Kokkos::View<Real_t*>("eos_work",         n);
            p_new        = Kokkos::View<Real_t*>("eos_p_new",        n);
            e_new        = Kokkos::View<Real_t*>("eos_e_new",        n);
            q_new        = Kokkos::View<Real_t*>("eos_q_new",        n);
            bvc          = Kokkos::View<Real_t*>("eos_bvc",          n);
            pbvc         = Kokkos::View<Real_t*>("eos_pbvc",         n);
         }
      } m_eosTemps;
   } m_elems;
