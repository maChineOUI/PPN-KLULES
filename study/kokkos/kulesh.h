...
typedef Int4_t  Index_t ;
typedef real8   Real_t ;
typedef Int4_t  Int_t ;
...
namespace lulesh_kokkos {
  ...
  template<class T> using HostView2D = Kokkos::View<T**, Kokkos::LayoutRight, Kokkos::HostSpace>;
}
...
public:
  ...
  {
     m_x   = View1D<Real_t>("m_x", numNode);
     m_y   = View1D<Real_t>("m_y", numNode);
     m_z   = View1D<Real_t>("m_z", numNode);
     m_xd  = View1D<Real_t>("m_xd", numNode);
     m_yd  = View1D<Real_t>("m_yd", numNode);
     m_zd  = View1D<Real_t>("m_zd", numNode);
     m_xdd = View1D<Real_t>("m_xdd", numNode);
     m_ydd = View1D<Real_t>("m_ydd", numNode);
     m_zdd = View1D<Real_t>("m_zdd", numNode);
     m_fx  = View1D<Real_t>("m_fx", numNode);
     m_fy  = View1D<Real_t>("m_fy", numNode);
     m_fz  = View1D<Real_t>("m_fz", numNode);
     m_nodalMass = View1D<Real_t>("m_nodalMass", numNode);
  }

  void AllocateElemPersistent(Int_t numElem)
  {
     m_nodelist = View2D<Index_t>("m_nodelist", numElem, 8);
     m_lxim = View1D<Index_t>("m_lxim", numElem);
     m_lxip = View1D<Index_t>("m_lxip", numElem);
     m_letam = View1D<Index_t>("m_letam", numElem);
     m_letap = View1D<Index_t>("m_letap", numElem);
     m_lzetam = View1D<Index_t>("m_lzetam", numElem);
     m_lzetap = View1D<Index_t>("m_lzetap", numElem);
     m_elemBC = View1D<Int_t>("m_elemBC", numElem);
     m_e = View1D<Real_t>("m_e", numElem);
     m_p = View1D<Real_t>("m_p", numElem);
     m_q = View1D<Real_t>("m_q", numElem);
     m_ql = View1D<Real_t>("m_ql", numElem);
     m_qq = View1D<Real_t>("m_qq", numElem);
     m_v = View1D<Real_t>("m_v", numElem);
     m_volo = View1D<Real_t>("m_volo", numElem);
  ...
  KOKKOS_INLINE_FUNCTION Real_t& x(Index_t idx)    { return m_x(idx) ; }
  KOKKOS_INLINE_FUNCTION Real_t& y(Index_t idx)    { return m_y(idx) ; }
  KOKKOS_INLINE_FUNCTION Real_t& z(Index_t idx)    { return m_z(idx) ; }

  KOKKOS_INLINE_FUNCTION Real_t& xd(Index_t idx)   { return m_xd(idx) ; }
  KOKKOS_INLINE_FUNCTION Real_t& yd(Index_t idx)   { return m_yd(idx) ; }
  KOKKOS_INLINE_FUNCTION Real_t& zd(Index_t idx)   { return m_zd(idx) ; }

  KOKKOS_INLINE_FUNCTION Real_t& xdd(Index_t idx)  { return m_xdd(idx) ; }
  KOKKOS_INLINE_FUNCTION Real_t& ydd(Index_t idx)  { return m_ydd(idx) ; }
  KOKKOS_INLINE_FUNCTION Real_t& zdd(Index_t idx)  { return m_zdd(idx) ; }

  KOKKOS_INLINE_FUNCTION Real_t& fx(Index_t idx)   { return m_fx(idx) ; }
  KOKKOS_INLINE_FUNCTION Real_t& fy(Index_t idx)   { return m_fy(idx) ; }
  KOKKOS_INLINE_FUNCTION Real_t& fz(Index_t idx)   { return m_fz(idx) ; }

  KOKKOS_INLINE_FUNCTION Real_t& nodalMass(Index_t idx) { return m_nodalMass(idx) ; }

  KOKKOS_INLINE_FUNCTION Index_t symmX_at(Index_t idx) const { return m_symmX_host.at(idx); }
  KOKKOS_INLINE_FUNCTION Index_t symmY_at(Index_t idx) const { return m_symmY_host.at(idx); }
  KOKKOS_INLINE_FUNCTION Index_t symmZ_at(Index_t idx) const { return m_symmZ_host.at(idx); }
  ...
  KOKKOS_INLINE_FUNCTION Index_t regElemSize_at(Index_t idx) const { return m_regElemSize(idx); }
  KOKKOS_INLINE_FUNCTION Index_t regNumList_at(Index_t idx) const { return m_regNumList(idx); }
  ...
  KOKKOS_INLINE_FUNCTION Index_t regElemlist_at(Int_t r, Index_t idx) const { return m_regElemlist_host[r][idx]; }

  KOKKOS_INLINE_FUNCTION Index_t nodelist_at(Index_t ei, int n) const { return m_nodelist(ei,n); }

  KOKKOS_INLINE_FUNCTION Index_t lxim_at(Index_t idx) const { return m_lxim(idx); }
  KOKKOS_INLINE_FUNCTION Index_t lxip_at(Index_t idx) const { return m_lxip(idx); }
  KOKKOS_INLINE_FUNCTION Index_t letam_at(Index_t idx) const { return m_letam(idx); }
  KOKKOS_INLINE_FUNCTION Index_t letap_at(Index_t idx) const { return m_letap(idx); }
  KOKKOS_INLINE_FUNCTION Index_t lzetam_at(Index_t idx) const { return m_lzetam(idx); }
  KOKKOS_INLINE_FUNCTION Index_t lzetap_at(Index_t idx) const { return m_lzetap(idx); }

  KOKKOS_INLINE_FUNCTION Int_t& elemBC_at(Index_t idx) { return m_elemBC(idx); }

  KOKKOS_INLINE_FUNCTION Real_t& dxx_at(Index_t idx)  { return m_dxx(idx) ; }
  KOKKOS_INLINE_FUNCTION Real_t& dyy_at(Index_t idx)  { return m_dyy(idx) ; }
  KOKKOS_INLINE_FUNCTION Real_t& dzz_at(Index_t idx)  { return m_dzz(idx) ; }

  KOKKOS_INLINE_FUNCTION Real_t& vnew_at(Index_t idx)  { return m_vnew(idx) ; }

  KOKKOS_INLINE_FUNCTION Real_t& delv_xi_at(Index_t idx)    { return m_delv_xi(idx) ; }
  KOKKOS_INLINE_FUNCTION Real_t& delv_eta_at(Index_t idx)   { return m_delv_eta(idx) ; }
  KOKKOS_INLINE_FUNCTION Real_t& delv_zeta_at(Index_t idx)  { return m_delv_zeta(idx) ; }

  KOKKOS_INLINE_FUNCTION Real_t& delx_xi_at(Index_t idx)    { return m_delx_xi(idx) ; }
  KOKKOS_INLINE_FUNCTION Real_t& delx_eta_at(Index_t idx)   { return m_delx_eta(idx) ; }
  KOKKOS_INLINE_FUNCTION Real_t& delx_zeta_at(Index_t idx)  { return m_delx_zeta(idx) ; }

  KOKKOS_INLINE_FUNCTION Real_t& e_at(Index_t idx)          { return m_e(idx) ; }
  KOKKOS_INLINE_FUNCTION Real_t& p_at(Index_t idx)          { return m_p(idx) ; }
  KOKKOS_INLINE_FUNCTION Real_t& q_at(Index_t idx)          { return m_q(idx) ; }

  KOKKOS_INLINE_FUNCTION Real_t& ql_at(Index_t idx)         { return m_ql(idx) ; }
  KOKKOS_INLINE_FUNCTION Real_t& qq_at(Index_t idx)         { return m_qq(idx) ; }

  KOKKOS_INLINE_FUNCTION Real_t& v_at(Index_t idx)          { return m_v(idx) ; }
  KOKKOS_INLINE_FUNCTION Real_t& delv_at(Index_t idx)       { return m_delv(idx) ; }

  KOKKOS_INLINE_FUNCTION Real_t& volo_at(Index_t idx)       { return m_volo(idx) ; }
  KOKKOS_INLINE_FUNCTION Real_t& vdov_at(Index_t idx)       { return m_vdov(idx) ; }

  KOKKOS_INLINE_FUNCTION Real_t& arealg_at(Index_t idx)     { return m_arealg(idx) ; }

  KOKKOS_INLINE_FUNCTION Real_t& ss_at(Index_t idx)         { return m_ss(idx) ; }

  KOKKOS_INLINE_FUNCTION Real_t& elemMass_at(Index_t idx)  { return m_elemMass(idx) ; }

  KOKKOS_INLINE_FUNCTION Index_t nodeElemCount_at(Index_t idx) const
  { return static_cast<Index_t>(m_nodeElemStart_host_vector[idx+1] - m_nodeElemStart_host_vector[idx]) ; }

  Index_t *nodeElemCornerList_ptr_host(Index_t idx)
...
private:
  ...
  View2D<Index_t> m_nodelist ;

  View1D<Index_t> m_lxim ;
  View1D<Index_t> m_lxip ;
  View1D<Index_t> m_letam ;
  View1D<Index_t> m_letap ;
  View1D<Index_t> m_lzetam ;
  View1D<Index_t> m_lzetap ;

  View1D<Int_t> m_elemBC ;
  std::vector<Int_t> m_elemBC_host ;
