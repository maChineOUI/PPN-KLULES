                               ...
                               Int_t numRanks)
{
   ...
   // KOKKOS CHANGE: Use host accessor e_host() to avoid GPU segfault
   // Assumes data has been deep_copied back to host before this function is called.
   std::cout << std::setw(12) << locDom.e_host(ElemId) << "\n";

   Real_t   MaxAbsDiff = Real_t(0.0);
   Real_t TotalAbsDiff = Real_t(0.0);
   Real_t   MaxRelDiff = Real_t(0.0);

   // This loop runs on CPU (Host), so we must access Host Mirrors
   for (Index_t j=0; j<nx; ++j) {
      for (Index_t k=j+1; k<nx; ++k) {
         // KOKKOS CHANGE: locDom.e() -> locDom.e_host()
         Real_t val1 = locDom.e_host(j*nx+k);
         Real_t val2 = locDom.e_host(k*nx+j);
         
         Real_t AbsDiff = FABS(val1 - val2);
         TotalAbsDiff  += AbsDiff;

         if (MaxAbsDiff <AbsDiff) MaxAbsDiff = AbsDiff;

         Real_t RelDiff = AbsDiff / val2;
